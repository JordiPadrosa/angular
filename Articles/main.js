...
function agregarFila() {
    if(document.getElementsByName("articles")[0].value == "Llibre"){
        if(document.getElementsByName("Llibre")[0]){
            ...
            document.getElementById("+"+article.nom).addEventListener("click", () => {
                sumarQuantitat(article);
                canviarQuantitat(article);
                calcularResultats(article);
            });
            document.getElementById("-"+article.nom).addEventListener("click", () => {
                restarQuantitat(article);
                canviarQuantitat(article);
                calcularResultats(article);
            });
            calcularResultats(article);
...
function escriureFila(article){
    ...
    '<td class="fila'+article.nom+'">'+article.codi+'</td><td>'+article.nom+'</td><td><input type="number" name="'+article.nom+'" value="1"></td><td>'+article.preu+'</td><td class="total" name="total'+article.nom+'">'+article.total+'</td><td><input name="descompte'+article.nom+'" value="0"></td><td><button id="+'+article.nom+'">+</button><button id="-'+article.nom+'">-</button></td>';
}
...
function sumarQuantitat(article) {
    document.getElementsByName(article.nom)[0].value = parseInt(document.getElementsByName(article.nom)[0].value) +1;
}

function restarQuantitat(article) {
    if(parseInt(document.getElementsByName(article.nom)[0].value) > 1){
        document.getElementsByName(article.nom)[0].value = parseInt(document.getElementsByName(article.nom)[0].value) -1;
    }
}

function aplicarDescompte(article) {
    document.getElementsByName("baseImposable")[0].textContent = suma*document.getElementsByName("descompte"+article.nom).value/100 +"€";
    document.getElementsByName("iva")[0].textContent = (21*suma/100)+"€";
    document.getElementsByName("import")[0].textContent = (suma + 21*suma/100)*document.getElementsByName("descompte"+article.nom.value)/100+"€";
}