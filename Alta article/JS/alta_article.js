window.onload = function () {
    ...
    document.getElementsByName("edifici")[0].onchange = comprovarEdifici;
...
function buscarLocalitzacions() {
    var localitzacio = ["Madrid", "Barcelona", "Sevilla", "Lleida", "Mallorca", "Besalú"];
    return localitzacio;
}

function localitzacio() {
    let localitzacio = buscarLocalitzacions();
    var select = document.getElementsByName("localitzacio")[0];
    for (value in localitzacio) {
        var option = document.createElement("option");
        option.text = localitzacio[value];
        select.appendChild(option);
    }
}

function comprovarEdifici() {
    var cincLletres = document.getElementsByName("localitzacio")[0].value.substr(0, 5);
    var regex = new RegExp('^' + cincLletres + '[+][0-9]{3}.[a-z]{2}$');
    console.log(regex.test(document.getElementsByName("edifici")[0].value));
    if (regex.test(document.getElementsByName("edifici")[0].value)) {
        document.getElementsByClassName("imatgeEdifici")[0].src = "Imatges/tick.png";
    }
    else {
        document.getElementsByClassName("imatgeEdifici")[0].src = "Imatges/creu.png";
    }
}